...
public class ImperialGroceryListStrategy implements GroceryListStrategy {
    ...
    public List<Ingredient> combine(List<Ingredient> ingredients) {
        ...
        for (Accumulator accumulator : summed.values()) {
            result.add(formatImperial(accumulator.displayName, accumulator.dimension, accumulator.metricBaseAmount));
        }
    ...
    private static Ingredient formatImperial(String name, Dimension dimension, double metricBaseAmount) {
        if (dimension == Dimension.WEIGHT) {
            double ounces = Unit.OZ.fromMetricBase(metricBaseAmount);
            if (ounces >= 16.0) {
                return new Ingredient(name, round3(ounces / 16.0), "lb");
            }
            return new Ingredient(name, round3(ounces), "oz");
        }

        double flOz = Unit.FL_OZ.fromMetricBase(metricBaseAmount);
        return new Ingredient(name, round3(flOz), "fl oz");
    }
